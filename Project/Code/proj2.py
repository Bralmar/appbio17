...
def tred(counter, Namelist, finlist, Non_CleanList_Done):		
	...
	with open(str(counter), 'w') as kd:					#Creates a treefile using command "fnj" for each alignment and outputs it as a file with name as a number. 
		tree=check_output(["fnj", "-O", "newick", "fastprot_text"])
		kd.write(tree)
	
	tns = dendropy.TaxonNamespace()
	t1 = dendropy.Tree.get(file=open('asymmetric_0.5.tree', 'r'), schema="newick", tree_offset=0, taxon_namespace=tns) #Opens original tree and defines it as t1
	t2 = dendropy.Tree.get(file=open(str(counter), 'r'), schema="newick", tree_offset=0, taxon_namespace=tns)		#Opens each tree made from aligments as t2 
...
def main():
	...
	longlist = []
	counter=0
	for filename in glob.glob(os.path.join(path, '*.msl')): 		#Opens one file at a time with the .msl format in a selected folder typed as sys.argv[1]. 
		counter+=1
		#Remove stuff
